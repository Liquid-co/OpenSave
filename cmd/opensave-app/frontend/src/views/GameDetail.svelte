<script>
  ...
  // What Steam says the typed number is — shown under the field as it is
  ...
  let appIdCheck = null; // null | { state: 'checking'|'found'|'unknown'|'unreachable', name? }
  let appIdCheckTimer = null;
  let appIdCheckedFor = '';
  $: scheduleAppIdCheck(cfg?.appId ?? '');

  function scheduleAppIdCheck(raw) {
    const id = String(raw).trim();
    clearTimeout(appIdCheckTimer);
    if (id === '' || !/^\d+$/.test(id)) {
      appIdCheck = null;
      appIdCheckedFor = '';
      return;
    }
    if (id === appIdCheckedFor) return;
    // Debounced: a lookup per keystroke would hit Steam for every partial
    // number on the way to the real one.
    appIdCheckTimer = setTimeout(() => checkAppId(id), 500);
  }

  async function checkAppId(id) {
    appIdCheck = { state: 'checking' };
    try {
      const res = await api.get(`/api/steam/app?appId=${encodeURIComponent(id)}`);
      // The field may have moved on while Steam was answering.
      if (String(cfg?.appId ?? '').trim() !== id) return;
      appIdCheckedFor = id;
      appIdCheck = res?.found
        ? { state: 'found', name: res.name }
        : { state: res?.reason === 'unreachable' ? 'unreachable' : 'unknown' };
    } catch {
      if (String(cfg?.appId ?? '').trim() !== id) return;
      appIdCheckedFor = id;
      appIdCheck = { state: 'unreachable' };
    }
  }
      ...
      <div class="card config-card">
        ...
        <div class="config-fields">
          ...
          <div class="field">
            ...
            {#if appIdCheck?.state === 'checking'}
              <span class="hint appid-check">Checking with Steam…</span>
            {:else if appIdCheck?.state === 'found'}
              <span class="hint appid-check ok">✓ Steam: <strong>{appIdCheck.name}</strong></span>
            {:else if appIdCheck?.state === 'unknown'}
            ...
            {:else if appIdCheck?.state === 'unreachable'}
              <span class="hint appid-check warn">
